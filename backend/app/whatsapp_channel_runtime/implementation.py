...
from ..repositories.whatsapp_runtime import find_message_by_external_id, find_outbox_by_provider_message_id, register_event_receipt, update_message_status_event, update_outbox_status_event
...
def register_whatsapp_event_receipt(
    ...
    *,
    ...
    return register_event_receipt(
        conn,
        organization_id=organization_id,
        event_key=event_key,
        status=_stringify(event.get("event_class") or "event"),
...
def apply_whatsapp_status_event(
    ...
    *,
    ...
    outbox = find_outbox_by_provider_message_id(conn, provider_message_id)
    message = find_message_by_external_id(conn, provider_message_id)
    ...
    if outbox:
        ...
        outbox = update_outbox_status_event(conn, outbox_id=outbox["id"], status=mapped_status, provider_response=response_payload, provider_status_code=200 if mapped_status != "failed" else 500) or outbox
    if message:
        ...
        _append_unique_event(status_events, callback_payload)
        message = update_message_status_event(conn, message_id=message["id"], status=mapped_status, metadata=metadata) or message
