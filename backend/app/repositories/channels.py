...
def get_whatsapp_number_by_phone_id(conn: sqlite3.Connection, phone_number_id: str) -> dict | None:
    return fetch_one(conn, "SELECT * FROM whatsapp_numbers WHERE phone_number_id = ?", (phone_number_id,))


def get_whatsapp_number_for_bot(conn: sqlite3.Connection, bot_id: str) -> dict | None:
