...
def handle(service, uow: UnitOfWork, *, organization_id: str, bot_id: str | None, entity_type: str | None, entity_id: str | None, outcome_event_id: str | None, limit: int, user: dict) -> dict[str, Any]:
    ensure_org_access(user, organization_id)
    require_permission(user, organization_id, "operations.read")
    rows = service._list_attribution_rows(
        uow.conn,
