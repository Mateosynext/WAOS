...
def handle(service, uow: UnitOfWork, *, organization_id: str, entity_type: str, entity_id: str, user: dict) -> dict[str, Any]:
    ...
    attribution_rows = service._list_attribution_rows(
        uow.conn,
    ...
    latest = service._serialize_scorecard(scorecards[0]) if scorecards else None
    return ok({
        "entity_type": entity_type,
        "entity_id": entity_id,
        "latest_scorecard": latest,
        "scorecards": [service._serialize_scorecard(row) for row in scorecards[:10]],
