...
def handle(service, uow: UnitOfWork, *, user: dict, organization_id: str | None, bot_id: str | None, status: str | None, sort: str | None, limit: int, offset: int) -> list[dict]:
    rows = service._base_list_rows(uow.conn, user=user, organization_id=organization_id, bot_id=bot_id, status=status, sort=sort, limit=limit, offset=offset)
    return [service._decorate_item(row) for row in rows]
