...
from ...schemas import AgentRouteRequest, ApiEnvelope, FlexibleSchema
from ..dependencies import CurrentUoW, CurrentUser

router = APIRouter(tags=["agent_orchestration"])


@router.get("/api/v1/agent-orchestration/specialists", response_model=ApiEnvelope[FlexibleSchema])
...
@router.post("/api/v1/agent-orchestration/route", response_model=ApiEnvelope[FlexibleSchema])
...
@router.get("/api/v1/agent-orchestration/conversations/{conversation_id}", response_model=ApiEnvelope[FlexibleSchema])
