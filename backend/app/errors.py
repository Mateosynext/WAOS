...
def app_error_from_validation(exc: RequestValidationError) -> ValidationAppError:
    return ValidationAppError(details={"errors": exc.errors()})


def error_body(error: AppError, *, request: Request | None = None) -> dict[str, Any]:
    metadata = _request_metadata(request)
    message = error.message if error.expose_detail or error.status_code < 500 else "Internal server error"
...
def error_response(error: AppError, *, request: Request | None = None) -> JSONResponse:
    return JSONResponse(status_code=error.status_code, content=error_body(error, request=request), headers=dict(error.headers or {}))
