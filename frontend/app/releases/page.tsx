...
export default async function ReleasesPage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  return (
    <Shell title="Release flow" subtitle="Aquí ya no se publica desde atajos sueltos: se solicita release, se aprueba, se publica y se deja trazabilidad del cambio." action={botId ? <Link href={`/bots/${botId}/studio`} className="primary-btn">Editar bot</Link> : <Link href="/bots" className="primary-btn">Crear</Link>}>
      ...
      <ContextTip>Tu rol visible ahora es {roleLabel(role)}. La publicación directa desde el detalle del bot se reemplazó por este flujo formal con request, aprobación y evidencia.</ContextTip>
      {!botId ? <EmptyActionState title="Primero selecciona un bot" description="Releases, trazabilidad y diferencias contra publicado necesitan un contexto explícito." primaryAction={<BotScopeSwitcher bots={bots} selectedBotId={botId} redirectTo="/releases?stage=all" />} secondaryAction={<Link href="/bots" className="secondary-btn">Ver bots</Link>} /> : <>
      ...
      <Section title="Semáforo de producción" subtitle="Este semáforo sí bloquea publish si el bot todavía no cumple lo crítico para salir." icon="alert">
