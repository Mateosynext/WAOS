...
export default async function Page({ params, searchParams }: { params: Promise<{ mode: string; slug?: string[] }>; searchParams?: Promise<SearchParams> }) {
  ...
  const model = await loadBotStudioRoute({ mode, botId: firstParam(resolvedParams?.bot), wizardId: firstParam(resolvedParams?.wizard_id), organizationId: firstParam(resolvedParams?.organization_id), verticalId: firstParam(resolvedParams?.vertical), subvertical: firstParam(resolvedParams?.subvertical), primaryObjective: firstParam(resolvedParams?.primary_objective), step: normalizedStep });
  if (!model.verticals.length) return <Shell title="Bot Studio" subtitle="El flujo por etapas necesita catálogo de industrias." action={<Link href="/verticals" className="secondary-btn">Abrir industrias</Link>}><EmptyActionState title="Catálogo no disponible" description="No llegaron verticales al frontend, así que Bot Studio no puede renderizar las nuevas rutas por paso." primaryAction={<Link href="/verticals" className="primary-btn">Revisar catálogo</Link>} /></Shell>;
